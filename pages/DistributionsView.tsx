...
//   };
    const loadData = async () => {
    const [dData, tData, sData, qData, sysData, groupData] = await Promise.all([
      apiService.getDistributions(),
      apiService.getDistributionTypes(),
      apiService.getSchedules(),
      apiService.getQueries(),
    ...
    const queriesWithSystems = qData.map(query => ({
      ...query,
      system: sysData.find(s => s.id === query.systemId)
    }));

    const schedulesWithGroups = sData.map(sched => ({
      ...sched,
      group: groupData.find(g => g.id === (typeof sched.group === 'string' ? sched.group : (sched.group as any)?.id)) || sched.group
    }));

    setDistributions(dData);
    setTypes(tData);
    setSchedules(schedulesWithGroups);
    setQueries(queriesWithSystems);
  };
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 md:gap-8">
                                ...
                                <div className="flex flex-col items-center gap-2 w-32 shrink-0">
                                    ...
                                    <div className="text-center">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase">קבוצה</p>
                                        <p className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate w-32">{schedule?.group?.name || 'ללא קבוצה'}</p>
                                    </div>
                                </div>
      ...
      return (
          <div className="space-y-8 animate-fadeIn">
              ...
              <div className="flex flex-col items-center gap-2 text-slate-400 text-sm mb-6">
                  ...
                  <div className="flex items-center gap-3 w-full p-3 bg-slate-50 dark:bg-slate-900/20 rounded-lg border border-slate-100 dark:border-slate-900">
                      <div className="bg-slate-100 dark:bg-slate-900/50 p-2 rounded-full text-slate-600 dark:text-slate-300"><Layers size={16}/></div>
                      <div className="flex-1">
                          <span className="text-xs text-slate-400 block uppercase tracking-wider font-bold">קבוצה</span>
                          <span className="font-semibold text-slate-800 dark:text-slate-200">{schedule.group?.name || 'ללא קבוצה'}</span>
                      </div>
                  </div>
