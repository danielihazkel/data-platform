  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">הגדרות וניהול משאבים</h1>
      </div>

      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden min-h-[600px] flex flex-col transition-colors duration-200">
        {/* Tabs Header */}
        <div className="flex border-b border-slate-200 dark:border-slate-700">
          <button
            ...
            className={`px-8 py-4 text-sm font-semibold flex items-center gap-2 transition-colors relative ${
              activeTab === 'systems' 
                ? 'text-[#664FE1] dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/20' 
          ...
          >
            <Server size={18} />
            מערכות (Systems)
            {activeTab === 'systems' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#664FE1] dark:bg-indigo-400"></div>}
          </button>
          <button
          ...
          >
            <Users size={18} />
            קבוצות תזמון (Groups)
            {activeTab === 'groups' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#664FE1] dark:bg-indigo-400"></div>}
          </button>
        </div>
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
            <div className="relative w-64">
                ...
                <input
                    type="text"
                    placeholder="חיפוש..."
                    className="w-full pl-4 pr-10 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#664FE1] focus:border-transparent text-slate-800 dark:text-slate-100"
                    value={searchTerm}
        ...
        <div className="flex-1 overflow-auto">
            {loading ? (
                ...
                <table className="w-full text-right text-sm">
                    <thead className="bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-semibold sticky top-0">
                        <tr>
                            <th className="px-6 py-3 border-b border-slate-200 dark:border-slate-700">מזהה</th>
                            <th className="px-6 py-3 border-b border-slate-200 dark:border-slate-700">שם</th>
                            <th className="px-6 py-3 border-b border-slate-200 dark:border-slate-700">אחראי (User Manager)</th>
                            <th className="px-6 py-3 border-b border-slate-200 dark:border-slate-700">מחלקה</th>
                            <th className="px-6 py-3 border-b border-slate-200 dark:border-slate-700">LOB</th>
                            <th className="px-6 py-3 border-b border-slate-200 dark:border-slate-700 w-20"></th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                        {filteredItems.map((item: any) => (
                            <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                                <td className="px-6 py-3 font-mono text-slate-500 dark:text-slate-400">{item.id}</td>
                                <td className="px-6 py-3 font-medium text-slate-800 dark:text-slate-200">{item.name}</td>
                                <td className="px-6 py-3 text-slate-600 dark:text-slate-400">{item.userManager || '-'}</td>
                                <td className="px-6 py-3 text-slate-600 dark:text-slate-400">{item.department || '-'}</td>
                                <td className="px-6 py-3 text-slate-600 dark:text-slate-400">{item.lineOfBusiness || '-'}</td>
                                <td className="px-6 py-3 text-left">
                                    <button 
                                        onClick={() => handleEdit(item)}
                                        className="p-2 text-slate-400 hover:text-[#664FE1] dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-full transition-all"
                                    >
                                        <Edit2 size={16} />
                                    </button>
                                </td>
      ...
      {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
              <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-xl shadow-2xl flex flex-col animate-scaleIn border border-slate-200 dark:border-slate-700">
                  <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 rounded-t-xl">
                      ...
                      {(() => {
                          ...
                          return (
                            <>
                                ...
                                <div className="space-y-1">
                                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">אחראי (User Manager)</label>
                                    <input
                                        className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">מחלקה</label>
                                        <input
                                            type="number"
                                            className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100"
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">LOB</label>
                                        <input
                                            type="number"
                                            className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100"
